<template>
	<div class="flex flex-col w-full gap-5" v-if="summary.data">
		<div class="text-lg text-ink-gray-8 font-bold">{{ __("Expense Claim Summary") }}</div>
		<div
			class="flex flex-col gap-4 bg-surface-white py-3 px-3.5 rounded-lg border-none"
		>
			<div class="flex flex-col gap-1.5">
				<span class="text-ink-gray-5 text-base font-medium leading-5">
					{{ __("Total Claimed Amount") }}
				</span>
				<span class="text-ink-gray-8 text-lg font-bold leading-6">
			...
			<div class="flex flex-row justify-between">
				<div class="flex flex-col gap-1">
					<div class="flex flex-row gap-1 items-center">
						<span class="text-ink-gray-5 text-sm font-medium leading-5">
							{{ __("Pending") }}
						</span>
						<FeatherIcon name="alert-circle" class="text-yellow-500 h-3 w-3" />
					</div>
					<span class="text-ink-gray-8 text-base font-semibold leading-6">
						{{
							formatCurrency(
