<template>
	<div class="flex flex-col w-full">
		<div class="flex flex-row justify-between items-center px-4">
			<div class="text-lg text-ink-gray-8 font-bold">{{ __("Leave Balance") }} </div>
			<router-link
		...
		>
			<div
				...
				:key="leave_type"
				class="flex flex-col bg-surface-white border-none rounded-lg drop-shadow-md gap-2 p-4 items-start first:ml-4"
			>
				<SemicircleChart
					:percentage="allocation.balance_percentage"
					:colorClass="getChartColor(index)"
				/>
				<div class="text-ink-gray-8 font-bold text-base">
					{{ `${allocation.balance_leaves}/${allocation.allocated_leaves}` }}
				</div>
				<div class="text-ink-gray-5 font-normal text-sm w-24 leading-4">
